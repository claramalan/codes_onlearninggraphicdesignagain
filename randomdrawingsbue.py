...
def draw_random_drawings(image_path, output_path, num_drawings):
    ...
    # Define the color for the drawings (blue)
    blue = (0, 0, 255)
    
    # Draw random drawings with blue color
    for _ in range(num_drawings):
        ...
        if drawing_type == "line":
            draw.line([x1, y1, x2, y2], fill=blue, width=random.randint(1, 5))
        elif drawing_type == "curve":
            # Example of drawing a curve
            draw.arc([x1, y1, x2, y2], start=random.randint(0, 180), end=random.randint(180, 360), fill=blue)
...
output_path = '/Users/claramalan/Desktop/drawingIMG_4590 .JPG'
num_drawings = 500  # Number of random drawings to draw
